...
|---|---|
| `tools/nx/plugin.ts` (the repo's ONLY registered nx plugin, **zero deps**) | one `createNodesV2` over one braced glob, dispatching to the six inference modules, plus a `createDependencies` for the crate-to-crate edges. One registration, not six: nx forks an isolated worker process per registered plugin, and three entries already measured 1.45s vs 1.20s user CPU per graph build for identical output. It also NAMES every crate node from `[package] name`: `@monodon/rust` resolves names and edges through `cargo metadata`, and a job with no Rust toolchain (CI's `affected`/`container`) would otherwise fail the whole graph with "projects … have no name provided". |
