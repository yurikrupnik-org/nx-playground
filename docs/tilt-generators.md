...
| `tools/nx/rust-targets.ts` | gives every cargo crate its `build` (+`production`) and, for a crate with a binary, `run` — `@monodon/rust` infers nodes and dep edges but only the `nx-release-publish` target. |
