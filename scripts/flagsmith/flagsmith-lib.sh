...
fs_wait_healthy() {
  ...
  fs_die "Flagsmith at $FLAGSMITH_URL never became healthy (last status $FS_STATUS).
       Start it with \`task docker-up\` and check \`docker logs flagsmith\`."
}
...
fs_discover() {
  ...
  [[ -n $org ]] || fs_die "organisation '$FLAGSMITH_ORG' not found — run \`task flags-bootstrap\` first"
  fs_admin GET "/api/v1/projects/?organisation=$org"
  fs_ok || fs_die "listing projects failed (HTTP $FS_STATUS): $FS_BODY"
  FS_PROJECT_ID=$(fs_find name "$FLAGSMITH_PROJECT" id)
  [[ -n $FS_PROJECT_ID ]] || fs_die "project '$FLAGSMITH_PROJECT' not found — run \`task flags-bootstrap\` first"
  fs_admin GET "/api/v1/environments/?project=$FS_PROJECT_ID"
  fs_ok || fs_die "listing environments failed (HTTP $FS_STATUS): $FS_BODY"
  FS_ENV_KEY=$(fs_find name "$FLAGSMITH_ENV_NAME" api_key)
  [[ -n $FS_ENV_KEY ]] || fs_die "environment '$FLAGSMITH_ENV_NAME' not found — run \`task flags-bootstrap\` first"
}
...
# to admin-API discovery — so the tasks work straight after bootstrap with
# nothing in .env.local yet. `dotenv: ['.env']` in Taskfile.yml means tasks
