#!/usr/bin/env bash
# Read and flip todo feature flags from the CLI. Invoked by `task flags-list`
# and `task flags-set`.
#
...
# real value; otherwise it is discovered through the admin API, so these tasks
# work straight after `task flags-bootstrap` with nothing in .env.local yet.
set -euo pipefail

# shellcheck source=scripts/flagsmith/flagsmith-lib.sh
...
$(printf '       %s\n' "${FS_FLAGS[@]%%|*}")
       Run \`task flags-bootstrap\` if the project has not been provisioned."
  printf '%s' "$id"
}
...
cmd_set() {
  ...
  if [[ -z $ident ]]; then
    ...
    [[ -n $fsid ]] || fs_die "no environment feature state for '$name' — re-run \`task flags-bootstrap\`"
