//! zerg API — B2B multi-tenant backend (WorkOS OIDC, gRPC tasks service, Qdrant vectors).
//!
//! The binary is a thin entrypoint; everything lives here so integration tests can
//! build state and routers directly. See `apps/zerg/api/src/main.rs`.
...
/// Boot the service: load config, connect dependencies, serve until shutdown.
pub async fn run() -> eyre::Result<()> {
    // Load configuration from environment variables
    let config = Config::from_env()?;

    // Initialize tracing with ErrorLayer for span trace capture.
    // Guard must outlive the server so OTEL spans flush before the runtime drops.
