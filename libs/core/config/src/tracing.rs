...
pub fn init_tracing(environment: &Environment, app: AppInfo) -> TracingGuard {
    ...
    // Install the W3C Trace Context propagator so outbound requests (e.g. the gRPC
    // pool's `TracingInterceptor`) can inject `traceparent`/`tracestate`, and inbound
    // servers can extract them — stitching spans into one trace across services.
    // Safe to set unconditionally: with no OTEL layer active the injected context is
    // empty, so this is a no-op rather than a behaviour change.
